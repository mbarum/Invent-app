...
const Quotations: React.FC = () => {
    ...
    const handleOpenCreateModal = async () => {
        ...
        try {
            if (allCustomers.length === 0) setAllCustomers(await getCustomers());
            if (allProducts.length === 0) setAllProducts(await getProducts());
            const today = new Date();
            today.setDate(today.getDate() + 7); // Default validity: 7 days
            setValidUntil(today.toISOString().split('T')[0]);
        } catch {
            toast.error("Could not load customers/products for new quotation.");

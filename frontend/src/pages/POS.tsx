...
import { createSale, getInvoices, getInvoiceDetails, initiateMpesaPayment, getMpesaPaymentStatus } from '../services/api.ts';
import { User, Search, X, Plus, Minus, Printer, LoaderCircle, FileText, Ban, Download, CheckCircle, XCircle } from 'lucide-react';
...
const POS: React.FC = () => {
    ...
    const { products: allProducts, customers: allCustomers, isInitialDataLoaded, refetchProducts, refetchSales } = useDataStore();
    ...
    const handleMpesaModalClose = () => {
        ...
        if (mpesaStatus === 'success') {
            // Receipt modal will be shown via completedSale state, so don't reset yet
        }
    };
      ...
      <>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <Card><CardHeader><CardTitle>Pay an Invoice</CardTitle><CardDescription>Search for an unpaid invoice to load it for payment.</CardDescription></CardHeader><CardContent>
                         <div className="relative">
                            ...
                            <Input placeholder="Search by invoice number or customer name..." className="pl-10" value={invoiceSearch} onChange={e => setInvoiceSearch(e.target.value)} disabled={!isInitialDataLoaded || !!payingForInvoice} />
                        </div>
                    ...
                    <Card><CardHeader><CardTitle>Product Search (Ctrl+I)</CardTitle></CardHeader><CardContent>
                        <div className="relative">
                            ...
                            <Input ref={productSearchRef} placeholder="Search by part number or name..." className="pl-10" value={productSearch} onChange={e => setProductSearch(e.target.value)} disabled={!isInitialDataLoaded || !!payingForInvoice} />
                        </div>
                     ...
                     <Card><CardHeader><CardTitle>Customer (Ctrl+U)</CardTitle></CardHeader><CardContent>
                         {selectedCustomer ? (
                            ...
                            <div className="relative">
                                ...
                                <Input ref={customerSearchRef} placeholder="Search by name or phone..." className="pl-10" value={customerSearch} onChange={e => setCustomerSearch(e.target.value)} disabled={!isInitialDataLoaded || !!payingForInvoice}/>
                        ...
                        <CardContent>
                            {cart.length === 0 ? <p className="text-gray-400 text-center py-8">Cart is empty</p> : (
                                <>
                                    ...
                                    {payingForInvoice && <Button variant="secondary" size="sm" className="w-full mt-4" onClick={() => { resetSale(); }}><Ban className="h-4 w-4 mr-2"/> Cancel Invoice Payment</Button>}
        ...
        <Modal isOpen={isMpesaModalOpen} onClose={handleMpesaModalClose} title="M-Pesa Payment">
            ...
            {mpesaStatus === 'success' && (<div className="text-center py-8">
                ...
                <p className="text-gray-400">Sale has been recorded. Closing this window will show the receipt.</p>
                <Button className="mt-4" onClick={handleMpesaModalClose}>View Receipt</Button>
            </div>)}
        ...
        {completedSale && (
            <Modal isOpen={!!completedSale} onClose={resetSale} title={`Receipt for ${completedSale.sale_no}`}>

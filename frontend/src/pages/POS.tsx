...
const POS: React.FC = () => {
    ...
    const { products: allProducts, customers: allCustomers, isInitialDataLoaded } = useDataStore();
    ...
    const removeFromCart = (productId: string) => {
        if (payingForInvoice) { toast.error("Cannot modify cart while paying for an invoice."); return; }
    ...
    const { subtotal, discountAmount, taxAmount, total } = useMemo(() => {
        ...
        let disc = 0;
        if (!payingForInvoice) {
            if (discountType === 'percent') disc = sub * (discount / 100);
            else disc = discount;
        }
    ...
    const handleSelectInvoice = async (invoice: Invoice) => {
        try {
            ...
            const cartItems: CartItem[] = (detailedInvoice.items || []).map(item => ({
                product: {
                    id: item.product_id,
                    partNumber: item.part_number || 'N/A', name: item.product_name || 'Unknown Product',
    ...
    const completeNonMpesaSale = async () => {
        ...
        try {
            ...
            resetSale();
        } catch (err: any) { toast.error(`Failed to complete sale: ${err.message}`);
    ...
    const handleRequestMpesaPayment = async () => {
        ...
        try {
            const payload = {
                ...
                cart, customerId: selectedCustomer!.id, branchId: currentBranch.id, invoiceId: payingForInvoice?.id
            };
            const { checkoutRequestId } = await initiateMpesaPayment(payload);
            setMpesaCheckoutId(checkoutRequestId);
        } catch (err: any) { setMpesaStatus('failed'); setMpesaError(err.message || "Failed to initiate payment."); }
    };
    
    const handleMpesaModalClose = () => {
        setIsMpesaModalOpen(false);
        if (mpesaStatus === 'success') { resetSale(); }
    };
    ...
    const handleDownloadPdf = async () => {
        ...
        } catch (error) { console.error(error); toast.error('Failed to generate PDF.', { id: toastId }); }
    };
